...
## Measurement pitfall: a wall-clock A/B cannot see a 5% kernel effect
...
**The general rule this produced:** if the effect is a few percent of a kernel
that is a fraction of GPU busy, measure the *kernel*
(`tests/FakeData/fringetile-sweep.sh`, or `ncu-frac.sh`), not a correlation. The
kernel-level sweep answered in one 2-minute allocation what two correlator jobs
could not, and it is immune to node sharing, srun overhead and leg ordering. Keep
wall-clock runs for what only they can give: an absolute number for the ledger or
the cost/benefit artifact, on a near-exclusive node.
...
The 2026-07-21 A100 capture at `7b8e31104` (host-tail overlap) - the "before"
for every idle-fraction comparison above - is in
[`gpu-profiling-archive.md`](gpu-profiling-archive.md), along with the older
captures it superseded.
